...
def run_lstm_pipeline(selected_features, df, sequence_length=30):
    ...
    def create_sequences(df_slice, fit_scaler=False):
        ...
        for i in range(sequence_length, len(scaled_vals)):
            ...
            y_seq.append(data_vals[i][0])  # Close is at index 0

        return np.array(X_seq), np.array(y_seq).reshape(-1, 1)

    X_train, y_train_raw = create_sequences(train_df, fit_scaler=True)
    X_test, y_test_raw = create_sequences(test_df, fit_scaler=False)
    ...
    return rmse, mae, mape, dir_acc, y_actual.flatten(), y_pred.flatten()
...
for features in feature_sets:
    ...
    rmse, mae, mape, dir_acc, y_actual, y_pred = run_lstm_pipeline(features, df)
    results.append((label, rmse, mae, mape, dir_acc))
    all_predictions[label] = (y_actual, y_pred)
...
output_csv_path = os.path.join(results_dir, "gru_results.csv")
results_df.sort_values("RMSE").to_csv(output_csv_path, index=False)
print(f"\nResults saved to: {output_csv_path}")

# Save plots
plots_dir = os.path.join(results_dir, "GRU_Plots")
os.makedirs(plots_dir, exist_ok=True)

for label, (y_actual, y_pred) in all_predictions.items():
